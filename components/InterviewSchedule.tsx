...
import { MOCK_INTERVIEWS, MOCK_CANDIDATES, MOCK_EMPLOYEES } from '../constants';
import { Interview } from '../types';
import { Calendar, Clock, Video, User, Plus, X, Check, Search, Filter, MoreHorizontal, Link as LinkIcon } from 'lucide-react';
...
const InterviewSchedule: React.FC = () => {
  ...
  const handleCreate = () => {
    if (!newInterview.candidateId || !newInterview.interviewerId || !newInterview.date || !newInterview.time) return;

    const candidate = MOCK_CANDIDATES.find(c => c.id === newInterview.candidateId);
    const interviewer = MOCK_EMPLOYEES.find(e => e.id === newInterview.interviewerId);
    ...
    if (candidate && interviewer) {
      const interview: Interview = {
        ...
        duration: newInterview.duration || 45,
        type: newInterview.type as any,
        status: 'Scheduled',
        meetingLink: 'https://meet.google.com/new-meeting-id'
      };
      setInterviews([...interviews, interview]);
      setIsModalOpen(false);
      setNewInterview({ date: new Date().toISOString().split('T')[0], time: '10:00', duration: 45, type: 'Technical', status: 'Scheduled' });
    }
  };
  ...
  const filteredInterviews = interviews.filter(i => filter === 'All' || i.status === filter).sort((a,b) => a.date.localeCompare(b.date) || a.time.localeCompare(b.time));
    ...
    <div className="h-full flex flex-col space-y-6">
      <div className="flex justify-between items-center">
        <div>
           ...
           <p className="text-slate-500">Manage upcoming technical and cultural interviews.</p>
        </div>
        <button 
             ...
             <div className="bg-slate-50 p-4 rounded-xl border border-slate-100">
                ...
                <div className="mt-4 pt-4 border-t border-slate-200">
                   <div className="flex justify-between text-xs text-slate-500 mb-1">
                      ...
                      <span className="font-bold text-slate-900">{interviews.filter(i => i.date === new Date().toISOString().split('T')[0]).length}</span>
                   </div>
                </div>
             </div>
         </div>
         ...
         <div className="lg:col-span-3 space-y-4 overflow-y-auto pb-6">
            {filteredInterviews.length === 0 ? (
               ...
               filteredInterviews.map((interview) => (
                  <div key={interview.id} className="bg-white p-5 rounded-xl border border-slate-200 shadow-sm hover:shadow-md transition-shadow group relative">
                     <div className="flex flex-col md:flex-row justify-between md:items-center gap-4">
                        ...
                        <div className="flex items-center gap-4 min-w-[180px]">
                           <div className="flex flex-col items-center justify-center w-14 h-14 bg-indigo-50 text-indigo-700 rounded-lg border border-indigo-100">
                              <span className="text-xs font-bold uppercase">{new Date(interview.date).toLocaleDateString('en-US', { month: 'short' })}</span>
                              <span className="text-xl font-bold">{new Date(interview.date).getDate()}</span>
                           </div>
                           <div>
                              <div className="flex items-center gap-1.5 font-bold text-slate-800">
                                 <Clock size={16} className="text-slate-400" /> {interview.time}
                              </div>
                              <div className="text-xs text-slate-500">{interview.duration} mins</div>
                           </div>
                        </div>

                        {/* Candidate Info */}
                        <div className="flex-1">
                           <h4 className="font-bold text-slate-900 text-lg">{interview.candidateName}</h4>
                           <p className="text-sm text-slate-500 mb-1">{interview.role}</p>
                           <div className="flex items-center gap-2 text-xs text-slate-400">
                              <span className="bg-slate-100 px-2 py-0.5 rounded text-slate-600 font-medium">{interview.type}</span>
                        ...
                        <div className="flex items-center gap-3">
                           ...
                           <span className={`px-3 py-1 rounded-full text-xs font-bold uppercase ${getStatusColor(interview.status)}`}>
                              {interview.status}
                           </span>
         ...
         <div className="fixed inset-0 z-50 flex items-center justify-center bg-slate-900/50 backdrop-blur-sm p-4">
            <div className="bg-white rounded-xl w-full max-w-lg shadow-2xl overflow-hidden">
               ...
               <div className="p-6 space-y-4">
                  ...
                  <div>
                     <label className="block text-xs font-bold text-slate-500 uppercase mb-1">Interviewer</label>
                     <select 
               ...
               <div className="p-4 bg-slate-50 border-t border-slate-200 flex justify-end gap-3">
                  ...
                  <button 
                     ...
                     disabled={!newInterview.candidateId || !newInterview.interviewerId}
