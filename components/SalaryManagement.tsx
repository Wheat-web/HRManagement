      ...
      api.get('/Employee')
         .then((res) => {
            ...
            const formatted = res.data.map((e: any) => ({
               ...
               paymentFrequency: e.paymentFrequency ?? e.PaymentFrequency ?? 'Monthly',
...
const saveChange = async (finalAmount: number) => {
   ...
   try {
      ...
      // 🔥 reload history
      const res = await api.get(`/CompensationChange/by-employee/${selectedEmployee.id}`);
            ...
            <div className="lg:col-span-1 bg-white rounded-xl shadow-sm border border-slate-200 overflow-hidden flex flex-col h-full">
               <div className="p-4 border-b border-slate-200 bg-slate-50 flex justify-between items-center">
                  ...
                  <button
                     ...
                     className={`flex items-center gap-1.5 px-3 py-1.5 rounded-lg text-xs font-medium transition-colors border ${viewAsMonthly
                                 ...
                                 {changes.length === 0 ? (
                                    ...
                                    changes.map((record) => (
                                       ...
                                       >
                                          ...
                                          <td className="px-6 py-4">
                                             <span
                                                className={`px-2 py-0.5 rounded-full text-xs font-bold ${record.changeType
 === 'Structure Change'
