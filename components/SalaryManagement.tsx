...
const SalaryManagement: React.FC = () => {
  ...
  const [modalMode, setModalMode] = useState<"Structure" | "Payment">(
    "Structure",
  );
  ...
  const [actionType, setActionType] = useState<
    "Structure Change" | "Bonus" | "Overtime"
  >("Structure Change");
    ...
    api
      ...
      .then((res) => {
        ...
        const formatted = res.data.map((e: any) => ({
          id: String(e.empID ?? e.id), // ⭐ ALWAYS STRING
          name: e.name ?? e.Name,
          role: e.role ?? e.Role,
          department: e.department ?? e.Department ?? "",
          salary: Number(e.salary ?? e.Salary ?? 0),
          paymentFrequency:
            e.paymentFrequency ?? e.PaymentFrequency ?? "Monthly",
  ...
  const getAnnualEquivalent = (salary: number, freq: PaymentFrequency) => {
    switch (freq) {
      case "Hourly":
        return salary * 2080; // 40hrs * 52wks
      case "Daily":
        return salary * 260; // 5days * 52wks
      case "Weekly":
        return salary * 52;
      case "Monthly":
        return salary * 12;
    ...
    const fetchHistory = async () => {
      try {
        ...
        const res = await api.get(
          `/CompensationChange/by-employee/${selectedEmpId}`,
        );
        ...
        setChanges(sorted);
        console.log("History loaded:", sorted);
      } catch (err) {
        console.log(err);
  ...
  const handleProcess = async () => {
    ...
    const newChange: CompensationChange = {
      ...
      type: actionType,
      amount:
        actionType === "Bonus" || actionType === "Overtime" ? finalAmount : 0,
      previousSalary:
        actionType === "Structure Change" ? selectedEmployee.salary : undefined,
      newSalary: actionType === "Structure Change" ? finalAmount : undefined,
      newFrequency:
        actionType === "Structure Change" ? newFrequency : undefined,
      reason:
        reason ||
        (actionType === "Overtime"
          ? `${otHours} hours @ ${otMultiplier}x`
          : "Manual adjustment"),
  ...
  const saveChange = async (finalAmount: number) => {
    if (!selectedEmployee) return;
    ...
    try {
      const payload = {
        ...
        amount: actionType === "Structure Change" ? 0 : finalAmount,

        previousSalary:
          actionType === "Structure Change" ? selectedEmployee.salary : null,

        newSalary: actionType === "Structure Change" ? finalAmount : null,

        newFrequency: actionType === "Structure Change" ? newFrequency : null,
      ...
      // 🔥 SAVE
      await api.post("/CompensationChange/save", payload);

      showToast("Saved to database", "success");

      // 🔥 reload history
      const res = await api.get(
        `/CompensationChange/by-employee/${selectedEmployee.id}`,
      );

      const formatted = res.data.map((r: any) => ({
        ...r,
        changeDate: r.changeDate ?? r.ChangeDate,
        changeType: r.changeType ?? r.ChangeType,
      }));
      ...
      <div className="flex justify-between items-center">
        <div>
          ...
          <p className="text-slate-500">
            Manage base salaries, hourly rates, overtime, and bonuses.
          </p>
        </div>
      </div>
          ...
          <div className="flex-1 overflow-y-auto">
            {paginatedEmployees.map((emp) => {
              const displayAmount = viewAsMonthly
                ? Math.round(
                    getMonthlyEquivalent(emp.salary, emp.paymentFrequency),
                  )
                : emp.salary;

              const displayFreq = viewAsMonthly
                ? "Monthly (Est.)"
                ...
                >
                  <div className="flex justify-between items-start">
                    ...
                    <div className="text-right">
                      <span
                        className={`block text-sm font-mono font-semibold ${viewAsMonthly ? "text-emerald-600" : "text-slate-600"}`}
                      >
                            ...
                            <td className="px-6 py-4">
                              <span
                                className={`px-2 py-0.5 rounded-full text-xs font-bold ${
                                  record.changeType === "Structure Change"
                                    ? "bg-purple-100 text-purple-700"
                                    : record.changeType === "Overtime"
                              ...
                              <div className="flex flex-col">
                                ...
                                {record.changeType === "Structure Change" && (
                                  <span className="text-xs text-slate-400">
                                    {record.previousSalary?.toLocaleString()} →{" "}
                                    {record.newSalary?.toLocaleString()} (
                                    {record.newFrequency || "Annual"})
                                  </span>
                                )}
                              </div>
                            </td>
                ...
                <>
                  <div>
                    ...
                    <select
                      value={newFrequency}
                      onChange={(e) =>
                        setNewFrequency(e.target.value as PaymentFrequency)
                      }
                      ...
                      <div className="mt-2 p-2 bg-slate-50 rounded border border-slate-100 text-xs text-slate-500 space-y-1">
                        <div className="flex justify-between">
                          ...
                          <span className="font-medium">
                            $
                            {Math.round(
                              getAnnualEquivalent(Number(amount), newFrequency),
                            ).toLocaleString()}
                          </span>
                        </div>
                        <div className="flex justify-between">
                ...
                <>
                  <div className="bg-blue-50 p-3 rounded-lg border border-blue-100 flex justify-between items-center text-sm">
                    ...
                    <span className="font-bold text-blue-900">
                      ${getHourlyRate(selectedEmployee!).toFixed(2)}/hr
                    </span>
                  </div>
                  ...
                  <div className="pt-2 border-t border-slate-100 flex justify-between items-center">
                    ...
                    <span className="text-xl font-bold text-emerald-600">
                      $
                      {calculateOTAmount().toLocaleString(undefined, {
