...
const ShiftManagement: React.FC = () => {
  ...
  const loadShifts = async () => {
    ...
    try {
      ...
      const formatted = res.data.map((s: any) => ({
        id: s.id,
        name: s.shiftName,
        startTime: s.startTime.slice(0, 5), // remove seconds
  ...
  const handleShiftChange = (empId: string, shiftId: string) => {
    setEmployees(
      employees.map((e) => (e.id === empId ? { ...e, shiftId } : e)),
    );
  ...
  const handleDeleteShift = (e: React.MouseEvent, id: string) => {
    ...
    if (assignedCount > 0) {
      showToast(
        `Cannot delete shift. It is currently assigned to ${assignedCount} employees.`,
        "warning",
      );
      return;
    }
      ...
      <div className="flex items-start justify-between">
        <div>
          ...
          <p className="text-slate-500">
            Manage employee rosters and shift assignments.
          </p>
        </div>
          ...
          {shifts.map((shift) => (
            ...
            >
              <div className="flex justify-between items-center mb-2">
                ...
                <span
                  className={`px-2 py-0.5 rounded text-[10px] font-bold uppercase ${shift.color}`}
                >
                  Active
                </span>
              </div>
              ...
              <div className="mt-3 text-xs text-slate-400 flex items-center justify-between">
                ...
                <span className="font-bold text-slate-700">
                  {employees.filter((e) => e.shiftId === shift.id).length}
                </span>
              </div>
                    ...
                    return (
                      <tr key={emp.id} className="hover:bg-slate-50">
                        ...
                        <td className="px-6 py-4">
                          {currentShift ? (
                            <span
                              className={`inline-flex items-center gap-2 px-2 py-1 rounded-md text-xs font-medium ${currentShift.color}`}
                            >
                              <Clock size={12} />
                              {currentShift.name} ({currentShift.startTime} -{" "}
                ...
                <div className="flex gap-3">
                  {colorOptions.map((opt) => (
                    <button
                      key={opt.label}
                      onClick={() =>
                        setNewShift({ ...newShift, color: opt.value })
                      }
