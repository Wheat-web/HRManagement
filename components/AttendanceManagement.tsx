...
import { Upload, Calendar, CheckCircle, XCircle, Clock, Search, FileDown, AlertTriangle } from 'lucide-react';
  ...
  const handleFileUpload = (e: React.ChangeEvent<HTMLInputElement>) => {
    if (e.target.files && e.target.files[0]) {
      ...
      setTimeout(() => {
        ...
        const newRecords: AttendanceRecord[] = MOCK_EMPLOYEES.map((emp, index) => ({
    ...
    <div className="space-y-6 h-full flex flex-col">
       <div className="flex justify-between items-center">
         <div>
            ...
            <p className="text-slate-500">Track daily attendance, upload biometrics logs, and manage exceptions.</p>
         </div>
                 ...
                 {filteredAttendance.length === 0 ? (
                    <tr>
                       <td colSpan={7} className="px-6 py-12 text-center text-slate-400">
                          <div className="flex flex-col items-center gap-2">
                             ...
                             <p>No records found for this date. Upload CSV to populate.</p>
                          </div>
                       </td>
                    </tr>
                 ) : (
                    filteredAttendance.map(record => (
                      <tr key={record.id} className="hover:bg-slate-50">
                         ...
                         <td className="px-6 py-4 font-bold">{record.hoursWorked ? `${record.hoursWorked} hrs` : '-'}</td>
