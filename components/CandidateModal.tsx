...
const CandidateModal: React.FC<CandidateModalProps> = ({
  ...
  const handleAIAnalysis = async () => {
    ...
    try {
      ...
      const jobDesc =
        "Senior Frontend Engineer with React, TypeScript, and modern state management experience. Needs 5+ years experience and ability to lead small teams.";
      ...
      onAudit(
        "AI Analysis",
        `Performed match analysis for ${candidate.name}`,
        false,
      );
    } catch (e) {
      console.error(e);
  ...
  const handleAction = (action: "approve" | "reject") => {
    const isOverride =
      (action === "approve" && (candidate.matchScore || 0) < 60) ||
      (action === "reject" && (candidate.matchScore || 0) > 80);

    const newStage =
      action === "approve" ? CandidateStage.INTERVIEW : CandidateStage.REJECTED;
    ...
    <div className="fixed inset-0 z-50 flex items-center justify-center bg-slate-900/50 backdrop-blur-sm p-4">
      <div className="bg-white rounded-2xl w-full max-w-4xl max-h-[90vh] overflow-hidden shadow-2xl flex flex-col">
        ...
        <div className="p-6 border-b border-slate-200 flex justify-between items-start bg-slate-50">
          ...
          <button
            onClick={onClose}
            className="text-slate-400 hover:text-slate-600 p-2 rounded-full hover:bg-slate-200"
          >
            <X size={24} />
          </button>
        </div>
          ...
          <div className="md:w-2/5 bg-slate-50/50 p-6">
            <div className="bg-white rounded-xl shadow-sm border border-indigo-100 p-5 mb-6 relative overflow-hidden">
              ...
              <div className="flex justify-between items-center mb-4">
                ...
                {displayScore && (
                  <div
                    className={`text-2xl font-bold ${displayScore >= 80 ? "text-emerald-600" : "text-amber-600"}`}
                  >
                    {displayScore}/100
                  </div>
                )}
