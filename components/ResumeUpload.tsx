...
const ResumeUpload: React.FC<ResumeUploadProps> = ({
  ...
  const [jobCombos, setJobCombos] = useState<JobOpeningCombo[]>([]);
  ...
  const loadJobCombos = async () => {
    const data = await getJobOpeningCombo();
    setJobCombos(data);
  };
  ...
  const processFile = async (uploadedFile: File) => {
    ...
    reader.onload = async () => {
      try {
        ...
        setError(
          err.message ||
            "Failed to parse resume. Please ensure the file is a valid PDF, DOCX, or TXT document and try again.",
        );
      } finally {
        setIsParsing(false);
      }
  ...
  const handleConfirm = async () => {
    ...
    const newCandidate: Candidate = {
      ...
      resumeSummary:
        ...
        (manualResumeFile ? `Resume attached: ${manualResumeFile.name}` : ""),
    ...
    try {
      const response = await api.post("/Candidate", {
        ...
        targetRole: newCandidate.role,
        experienceYears: newCandidate.experience,
        skills: newCandidate.skills.join(","),
        professionalSummary: newCandidate.resumeSummary,
        stageId: 1,
        jobOpeningId: newCandidate.jobId,
      });

      const createdId = response.data.id;
      newCandidate.id = createdId;
                ...
                <div className="space-y-4">
                  ...
                  {isParsing && (
                    <div className="text-center py-8 text-slate-500 flex flex-col items-center gap-3">
                      ...
                      />
                      <p className="text-sm">
                        AI Agent is analyzing the resume...
                      </p>
                    </div>
                  )}
                    ...
                    <div className="bg-white border border-emerald-100 rounded-lg p-4 shadow-sm animate-in fade-in slide-in-from-bottom-2">
                      ...
                      <div className="space-y-3 text-sm">
                        ...
                        <div>
                          ...
                          <p className="text-xs text-slate-600 mt-1 leading-relaxed bg-slate-50 p-2 rounded">
                            {parsedData.resumeSummary}
                          </p>
                        </div>
                      </div>
                    </div>
            ...
            <div className="space-y-4">
              ...
              <div>
                ...
                <textarea
                  ...
                  onChange={(e) =>
                    setManualData({ ...manualData, summary: e.target.value })
                  }
