...
import { Employee, Candidate, CandidateStage, PaymentFrequency, Branch, Department, EmployeeDocument } from '../types';
import { Users, Plus, MapPin, MoreHorizontal, DownloadCloud, SlidersHorizontal, Trash2, Search, Filter, Eye, Edit2, FileText, Upload, Calendar, Mail, Phone, DollarSign, Briefcase, CreditCard, X, Save, User, CheckCircle } from 'lucide-react';
...
const EmployeeManagement: React.FC<EmployeeManagementProps> = ({ 
  ...
  const [activeTab, setActiveTab] = useState<'identity' | 'work' | 'finance' | 'documents'>('identity');
  ...
  const handleOpenView = (emp: Employee) => {
      ...
      setActiveTab('identity'); // Reuse tab state for View modal as well
  ...
  const handleSave = () => {
      ...
      if (formData.id) {
          // Update
          setEmployees(prev => prev.map(e => e.id === newEmp.id ? newEmp : e));
  ...
  // Helper to render Form Fields based on Active Tab
          ...
          case 'work':
              return (
                  <div className="space-y-4 animate-in fade-in slide-in-from-right-4">
                      ...
                      <div>
                          <label className="block text-xs font-bold text-slate-500 uppercase mb-1">Work Location (Detail)</label>
                          <input 
                  ...
                  <div className="space-y-4 animate-in fade-in slide-in-from-right-4">
                      ...
                      <div className="space-y-2">
                              ...
                              <div key={doc.id} className="flex items-center justify-between p-3 bg-white border border-slate-200 rounded-lg shadow-sm">
                                  <div className="flex items-center gap-3">
                                      ...
                                      <div>
                                          ...
                                          <p className="text-xs text-slate-400">{doc.uploadDate} • {doc.size}</p>
                                      </div>
                                  </div>
                          ...
                          {(formData.documents || []).length === 0 && (
                              <p className="text-center text-slate-400 text-xs py-4">No documents uploaded yet.</p>
                          )}
                      </div>
                  </div>
              );
                  ...
                  <div className="space-y-6 animate-in fade-in">
                      <div className="grid grid-cols-2 gap-y-6">
                          ...
                          <div>
                              ...
                              <p className="text-slate-800 font-medium">{emp.gender || '-'}</p>
                          </div>
                          <div className="col-span-2">
                              <p className="text-xs font-bold text-slate-400 uppercase">Address</p>
                              <p className="text-slate-800 font-medium">{emp.address || '-'}</p>
                          </div>
                      </div>
                      
                      {emp.emergencyContact && (
                          <div className="bg-slate-50 p-4 rounded-lg border border-slate-100">
                              <h4 className="text-xs font-bold text-slate-500 uppercase mb-2">Emergency Contact</h4>
                              <div className="flex justify-between items-center">
                                  <div>
                                      <p className="font-bold text-slate-800">{emp.emergencyContact.name}</p>
                                      <p className="text-xs text-slate-500">{emp.emergencyContact.relation}</p>
                                  </div>
                                  <p className="text-sm font-mono text-slate-700">{emp.emergencyContact.phone}</p>
                              </div>
                          </div>
                      )}
                  </div>
              ...
              return (
                  <div className="space-y-6 animate-in fade-in">
                      <div className="grid grid-cols-2 gap-y-6">
                          ...
                          <div>
                              <p className="text-xs font-bold text-slate-400 uppercase">Job Role</p>
                          ...
                          <div>
                              ...
                              <p className="text-slate-800 font-medium">{emp.employmentType || 'Full-time'}</p>
                          </div>
                          <div>
                          ...
                          <div>
                              <p className="text-xs font-bold text-slate-400 uppercase">Branch</p>
                              <p className="text-slate-800 font-medium">{getBranchName(emp.branchId)}</p>
                          </div>
                      </div>
                  </div>
              );
              ...
              return (
                  <div className="space-y-6 animate-in fade-in">
                      <div className="bg-emerald-50 border border-emerald-100 p-6 rounded-xl">
                          <p className="text-emerald-800 font-medium mb-1">Total Compensation</p>
                          <h2 className="text-3xl font-bold text-emerald-900">{emp.currency} {emp.salary.toLocaleString()} <span className="text-sm font-normal opacity-70">/ {emp.paymentFrequency}</span></h2>
                      </div>
                          ...
                          <div className="bg-white border border-slate-200 rounded-xl p-4 shadow-sm">
                              ...
                              <div className="grid grid-cols-2 gap-4 text-sm">
                                  <div>
                                      ...
                                      <p className="font-medium text-slate-900">{emp.bankDetails.bankName}</p>
                                  </div>
                                  <div>
                                      <p className="text-slate-500">Account Name</p>
                                      <p className="font-medium text-slate-900">{emp.bankDetails.accountName}</p>
                                  </div>
                                  <div>
                                      <p className="text-slate-500">Account Number</p>
                                      <p className="font-mono text-slate-900">****{emp.bankDetails.accountNumber.slice(-4)}</p>
                                  </div>
                                  <div>
                                      <p className="text-slate-500">Swift Code</p>
                                      <p className="font-mono text-slate-900">{emp.bankDetails.swiftCode || '-'}</p>
                                  </div>
                              </div>
                          </div>
                      ) : (
                          <p className="text-slate-400 text-sm italic">No bank details added.</p>
                      )}
                  </div>
              );
          case 'documents':
                  ...
                  <div className="space-y-4 animate-in fade-in">
                      {(emp.documents || []).length === 0 ? (
                          <div className="text-center py-12 bg-slate-50 rounded-xl border border-dashed border-slate-200">
                              ...
                              <p className="text-slate-500 text-sm">No documents attached to this profile.</p>
                          </div>
                      ) : (
    ...
    <div className="space-y-6">
      <div className="flex justify-between items-center">
        <div>
          ...
          <p className="text-slate-500">Manage workforce profiles, documents, and compliance data.</p>
        </div>
        <button 
                ...
                {filteredEmployees.length === 0 ? (
                    ...
                    <tr key={emp.id} className="hover:bg-slate-50 group">
                        <td className="px-6 py-4">
                            <div className="flex items-center gap-3">
                                <div className="w-9 h-9 bg-indigo-100 text-indigo-700 rounded-full flex items-center justify-center text-sm font-bold">{emp.name.charAt(0)}</div>
                                <div>
                                    <p className="font-bold text-slate-900">{emp.name}</p>
                                    <p className="text-xs text-slate-500">{emp.employmentType || 'Full-time'}</p>
                                </div>
                            </div>
                        </td>
                        <td className="px-6 py-4">
                        ...
                        <td className="px-6 py-4">
                            <p className="text-slate-600 flex items-center gap-1"><Mail size={12} /> {emp.email}</p>
                            <p className="text-slate-500 text-xs mt-1 flex items-center gap-1"><Phone size={12} /> {emp.phone || '-'}</p>
                        </td>
                        <td className="px-6 py-4">
                        ...
                        <td className="px-6 py-4 text-right">
                            <div className="flex items-center justify-end gap-2 opacity-0 group-hover:opacity-100 transition-opacity">
                                <button onClick={() => handleOpenView(emp)} className="p-1.5 text-slate-400 hover:text-indigo-600 hover:bg-indigo-50 rounded">
                                    <Eye size={16} />
                                </button>
                                <button onClick={() => handleOpenEdit(emp)} className="p-1.5 text-slate-400 hover:text-indigo-600 hover:bg-indigo-50 rounded">
                                    <Edit2 size={16} />
                                </button>
                            </div>
                        </td>
        ...
        <div className="fixed inset-0 z-50 flex items-center justify-center bg-slate-900/50 backdrop-blur-sm p-4">
          <div className="bg-white rounded-xl w-full max-w-2xl shadow-2xl overflow-hidden max-h-[90vh] flex flex-col animate-in fade-in zoom-in-95">
            <div className="p-4 border-b border-slate-100 flex justify-between items-center bg-slate-50">
              <h3 className="font-bold text-slate-800">{formData.id ? 'Edit Employee' : 'Add New Employee'}</h3>
            ...
            <div className="p-4 bg-slate-50 border-t border-slate-200 flex justify-end gap-3">
               <button onClick={() => setIsEditModalOpen(false)} className="px-4 py-2 text-slate-600 hover:text-slate-800 text-sm font-medium">Cancel</button>
               <button onClick={handleSave} className="px-4 py-2 bg-indigo-600 text-white rounded-lg hover:bg-indigo-700 text-sm font-medium flex items-center gap-2">
          ...
          <div className="fixed inset-0 z-50 flex items-center justify-center bg-slate-900/50 backdrop-blur-sm p-4">
            <div className="bg-white rounded-xl w-full max-w-2xl shadow-2xl overflow-hidden max-h-[90vh] flex flex-col animate-in fade-in zoom-in-95">
                ...
                <div className="p-6 bg-slate-900 text-white flex justify-between items-start">
                    <div className="flex gap-4">
                        ...
                        <div>
                            ...
                            <p className="text-indigo-200 text-sm">{selectedEmployee.role}</p>
                            <div className="flex gap-2 mt-2">
                                <span className="text-xs bg-slate-800 px-2 py-0.5 rounded text-slate-300 border border-slate-700">{selectedEmployee.department}</span>
                                <span className={`text-xs px-2 py-0.5 rounded font-bold ${selectedEmployee.status === 'Active' ? 'bg-emerald-500 text-white' : 'bg-slate-500 text-white'}`}>
                                    {selectedEmployee.status}
                                </span>
                            </div>
                        </div>
                    </div>
                    <button onClick={() => setIsViewModalOpen(false)} className="text-slate-400 hover:text-white bg-white/10 p-1.5 rounded-full"><X size={18} /></button>
                </div>
                ...
                <div className="p-4 bg-slate-50 border-t border-slate-200 flex justify-end gap-3">
                    ...
                    <button onClick={() => setIsViewModalOpen(false)} className="px-4 py-2 bg-slate-800 text-white rounded-lg hover:bg-slate-900 text-sm font-medium">
                        Close
                    </button>
                </div>
            </div>
